...
# PID Parameters
nJoint=12
kP=np.array([.1 for i in range(nJoint)])
kI=np.array([.03 for i in range(nJoint)])
kD=np.array([.1 for i in range(nJoint)])

e_old=np.array([0. for i in range(nJoint)])
E=np.array([0. for i in range(nJoint)])
uu=np.array([0. for i in range(nJoint)])
...
def evolve():
    ...
    e=r-positions
    E=E+e
    uu=kP*e+kI*E+kD*(e-e_old)
    e_old=e
    
    
    inpp.append(r[numRec])
    outp.append(positions[numRec])
    cont_inp.append(uu[numRec])

    if i_bl==750:
        data=np.hstack((np.array(inpp),np.array(outp),np.array(cont_inp),joints[numRec],PP,speed_))
        PickleIt(data,'inpOut')
